...
public class JedisUtil {
        ...
        private JedisUtil() {
            if (pool == null) {
                String ip = Redis_Config.REDISHOST;
                int port = Redis_Config.REDISPORT;
                String password =Redis_Config.REDISPASS;
    ...
    public Long lpush(String key, List<String> vls) {
        Jedis jedis = getJedis();
        String[] vs=new String[vls.size()];
        for(int i=0;i<vls.size();i++){
            vs[i]=vls.get(i);
        }
        return jedis.lpush(key, vs);
    }
